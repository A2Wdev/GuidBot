...
namespace Guid.Bot
{
    ...
    public class ShopDailog : ComponentDialog
    {
            ...
            return await stepContext.PromptAsync(
                ...
                new PromptOptions
                {
                    Prompt = MessageFactory.Text(
                        $"Found {new string[] { "install.AppName" }}. What is the name of the machine to install application?"),
                },
                cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await stepContext.Context.SendActivityAsync(
                $"Great, your request to install {new string[] { "install.AppName"}}   on  {new string[] { " install.MachineName" }} has been scheduled.",
                cancellationToken: cancellationToken);
